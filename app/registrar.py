...
class ChatGPTRegister:
    ...
    def __init__(self, proxy=None, tag: str = ""):
    ...
    def _json_headers(self, referer: str, with_sentinel: bool = False, flow=None):
        from main import _make_trace_headers, build_sentinel_token
        ...
        if with_sentinel:
            sentinel_token = build_sentinel_token(
                self.session,
                self.device_id,
                flow=flow or "auth_page",
                user_agent=self.ua,
                sec_ch_ua=self.sec_ch_ua,
                impersonate=self.impersonate,
            )
            if not sentinel_token:
                raise RuntimeError(f"无法获取 {flow or 'auth_page'} 的 sentinel token")
    ...
    def validate_otp(self, code: str):
        ...
        headers = self._json_headers(f"{self.AUTH}/email-verification")
    ...
    def load_about_you(self):
        url = f"{self.AUTH}/about-you"
        r = self.session.get(
            url,
            headers={
                "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
        ...
        self._log(
            "6.5 Load About You", "GET", url, r.status_code, {"final_url": str(r.url)}
        )
    ...
    def run_register(self, email, password, name, birthdate, mail_token):
        ...
        if "create-account/password" in final_path:
            ...
            logger.info(
                "跳到 OTP 验证阶段 (authorize 已触发 OTP，不再重复发送)", tag=self.tag
            )
    ...
    def _oauth_allow_redirect_extract_code(self, url: str, referer=None):
        ...
        try:
            ...
            for r in getattr(resp, "history", []) or []:
                ...
                if code:
                    logger.debug(
                        "allow_redirect 命中 history Location code", tag="OAuth"
                    )
                    return code
            ...
            if maybe_localhost:
                ...
                if code:
                    logger.debug(
                        "allow_redirect 从 localhost 异常提取 code", tag="OAuth"
                    )
                    return code
    ...
    def _oauth_follow_for_code(self, start_url: str, referer=None, max_hops: int = 16):
        ...
        for hop in range(max_hops):
            try:
                ...
                if maybe_localhost:
                    ...
                    if code:
                        logger.debug(
                            f"follow[{hop + 1}] 命中 localhost 回调", tag="OAuth"
                        )
            ...
            logger.debug(
                f"follow[{hop + 1}] {resp.status_code} {last_url[:140]}", tag="OAuth"
            )
    ...
    def _oauth_submit_workspace_and_org(self, consent_url: str):
        ...
        if not session_data:
            ...
            logger.debug(
                f"无法解码 oai-client-auth-session, cookies={cookie_names[:12]}",
                tag="OAuth",
            )
            return None
        ...
        logger.debug(
            f"workspace/select page={ws_page or '-'} next={(ws_next or '-')[:140]}",
            tag="OAuth",
        )
        ...
        if org_id:
            ...
            if resp_org.status_code == 200:
                ...
                logger.debug(
                    f"organization/select page={org_page or '-'} next={(org_next or '-')[:140]}",
                    tag="OAuth",
                )
                if org_next:
    ...
    def perform_codex_oauth_login_http(
        ...
        from main import (
            ...
            _wait_for_verification_email_impl,
        ...
        def _bootstrap_oauth_session():
            ...
            logger.debug(
                f"/oauth/authorize -> {r.status_code}, final={(final_url or '-')[:140]}, redirects={redirects}",
                tag="OAuth",
            )
            ...
            logger.debug(
                f"login_session: {'已获取' if has_login else '未获取'}", tag="OAuth"
            )

            if not has_login:
                logger.debug(
                    "未拿到 login_session，尝试访问 oauth2 auth 入口", tag="OAuth"
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"/api/oauth/oauth2/auth -> {r2.status_code}, final={(final_url or '-')[:140]}, redirects={redirects2}",
                        tag="OAuth",
                    )
                except Exception as e:
                ...
                logger.debug(
                    f"login_session(重试): {'已获取' if has_login else '未获取'}",
                    tag="OAuth",
                )
        ...
        def _post_authorize_continue(referer_url: str):
            ...
            if not sentinel_authorize:
                logger.debug(
                    "authorize_continue 的 sentinel token 获取失败", tag="OAuth"
                )
                return None
        ...
        if resp_continue.status_code == 400 and "invalid_auth_step" in (
            ...
            if resp_continue is None:
                return None
            logger.debug(
                f"/authorize/continue(重试) -> {resp_continue.status_code}", tag="OAuth"
            )
        ...
        logger.debug(
            f"continue page={page_type or '-'} next={(continue_url or '-')[:140]}",
            tag="OAuth",
        )
        ...
        logger.debug(
            f"verify page={page_type or '-'} next={(continue_url or '-')[:140]}",
            tag="OAuth",
        )
        ...
        if need_oauth_otp:
            ...
            while time.time() < otp_deadline and not otp_success:
                ...
                logger.debug(
                    f"/email-otp/validate -> {resp_otp.status_code}", tag="OAuth"
                )
                if resp_otp.status_code != 200:
                    logger.debug(
                        f"OTP 无效，继续尝试下一条: {resp_otp.text[:160]}", tag="OAuth"
                    )
                    continue
                ...
                logger.debug(
                    f"OTP 验证通过 page={page_type or '-'} next={(continue_url or '-')[:140]}",
                    tag="OAuth",
                )
                otp_success = True
                break
        ...
        if token_resp.status_code != 200:
            logger.debug(
                f"token 交换失败: {token_resp.status_code} {token_resp.text[:200]}",
                tag="OAuth",
            )
            return None
